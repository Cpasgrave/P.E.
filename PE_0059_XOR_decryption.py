...
as the plain text message, and the key is made up of random bytes.
...
The balance for this method is using a sufficiently
...
of three lower case characters. Using cipher.txt
...
charset = {ord(c) for c in '''2KszDhMvik1;AnoGy57eSO6Xx)Uju c9'.?ratqBCZ4W!PVwEpY(LHJRl8Ng0-:TIfQdFb3m,'''}
keys = []
for n in range(128):
    for pos in range(3):
        dec = []
        for i in range(0,150,3):
            dec.append(message[i+pos]^n)
        if not len(set(dec)-charset):
            print(''.join([chr(d) for d in dec]))
            keys.append((pos,n))

print(keys)
keys = [k for pos,k in sorted(keys)]
print('the keys are:',keys,''.join([chr(k) for k in keys]),'\n')
